...
export default function WatchList() {
    ...
    const displayFilms = async () => {
        ...
        const userWatchListedFilms = films.filter(film => userWatchList.includes(film.id));
        dispatch(ACTION.FETCH_START())
        try{
            dispatch(ACTION.FETCH_SUCCESS_TITLE(userWatchListedFilms))
            setIsLoading(false);
        }catch(err){
    ...
    return (
        <div className="p-12 bg-black text-white min-h-screen flex flex-col items-center">
            <h1 className="font-Gill text-center font-bold w-full mb-8 text-2xl">LES FILMS QUE VOUS COMPTEZ VOIR</h1>
            ...
            {store && store[0] == null ? <p className="mt-8">Oh ! Vous n'avez pas encore de films dans votre liste de films à voir.</p> : []}
